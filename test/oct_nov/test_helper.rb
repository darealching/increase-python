...
class OctNov::Test::SingletonClient < OctNov::Client
  include Singleton

  TEST_API_BASE_URL = ENV.fetch("TEST_API_BASE_URL", "http://localhost:4010")

  def initialize
    super(base_url: OctNov::Test::SingletonClient::TEST_API_BASE_URL, api_key: "My API Key")
  end
end
...
class OctNov::Test::ResourceTest < Minitest::Test
  def async?
  ...
  def before_all
    super
    @oct_nov = OctNov::Test::SingletonClient.instance
  end
